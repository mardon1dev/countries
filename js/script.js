...
function renderCountries(arr) {
  ...
  let countries = arr.map((country) => {
    return `
        ...
        <img class="w-full h-[200px] object-cover rounded-lg flag" src="${country.flag}" alt="${country.name}">
        <div class="country-info p-3 bg-gray-700 w-full text-white">
        ...
        <button class="likeButton ${country.isLiked ? " text-red-600 border-red-800 bg-white":"bg-gray-800 text-white border-gray-700"} border-2 w-[40px] h-[40px] flex items-center justify-center rounded-lg  hover:cursor-pointer" data-id =${country.id}>
        ...
        <button class="saveButton ${country.isSaved ? "bg-white text-gray-700 border-gray-700" : " text-white bg-gray-800"} border-2 w-[40px] h-[40px] flex items-center justify-center rounded-lg hover:cursor-pointer" data-id =${country.id}>
        <svg class="w-[50%]" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
function searchCapiptal(arr) {
  ...
  fiindCapital.addEventListener("change", (e) => {
    console.log(fiindCapital);
...
countries.addEventListener("click", (event) => {
  ...
  if (moreButton) {
    let countryId = moreButton.dataset.id;
    let country = countrys.find(item => item.id == countryId);
    ...
    extraInfo.addEventListener("click", (evt) => {
        if (evt.target.classList.contains("extra-info")) {
            extraInfo.classList.add("scale-0");
            body.classList.remove("scroll");
        }
    });
    extraInfo.classList.remove("scale-0");
  ...
  if (likeButton) {
    ...
    let likedCount = countrys.filter(item => item.isLiked == true);
    likeCounts.textContent = likedCount.length;
  }
  ...
  if (saveButton) {
    ...
    let savedCount = countrys.filter(item => item.isSaved == true);
    savedCounts.textContent = savedCount.length;
  }
});


let showSavedCountries = document.querySelector(".saveCountries");
showSavedCountries.addEventListener("click", () => {
  let savedCountries = countrys.filter(item => item.isSaved == true);
  if (savedCountries.length > 0) {
    renderCountries(savedCountries)
  }
  else{
    alert("No saved countries");
  }
})

let showLikedCountries = document.querySelector(".likeCountries");
showLikedCountries.addEventListener("click", () => {
  let likedCountries = countrys.filter(item => item.isLiked == true);
  if (likedCountries.length > 0) {
    renderCountries(likedCountries)
  }
  else{
    alert("No liked countries");
  }
})